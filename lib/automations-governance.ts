/** Shared Automations workload classification and model-routing policy. */
...
export function loadGovernanceConfig(cwd: string = process.cwd()): GovernanceConfig {
	...
	let profile = readPiSettingsKey("automationsProfile", projectSettingsPath);
	if (!profile) profile = readPiSettingsKey("automationsProfile", PI_SETTINGS_PATH);
