const SECRET_ASSIGNMENT_PATTERN = /(["']?\b(?=[A-Za-z0-9_.-]*(?:api[_-]?key|apikey|token|secret|password|passwd|pwd|credential))[A-Za-z][A-Za-z0-9_.-]*["']?\s*[:=]\s*)("[^"]{4,}"|'[^']{4,}'|[^\s,;)}\]]{4,})/gi;
const AUTH_HEADER_PATTERN = /(["']?\bAuthorization["']?\s*[:=]\s*["']?(?:Bearer|Basic)\s+)[^"'\s,;)}\]]+(?=["']?)/gi;
