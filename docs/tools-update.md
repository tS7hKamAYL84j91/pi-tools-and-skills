...
| TCU-002 `/name` vs `/alias` semantic overlap | Remove `/alias`; add `set_name`; sync registry via heartbeat | Phase 1 implemented: `/alias` removed, `set_name`/`get_name` added, deprecated alias wrappers retained, registry heartbeat sync added | ADR-010/011/012/013 accepted; council resolved RQ-1/2/3 | In Progress |
...
- 2026-05-04: Council debate resolved three primary open questions (RQ-1 heartbeat sync, RQ-2 `set_name` + deprecated `set_alias`, RQ-3 `/name` overrides spawn name). ADR-010/011 promoted to Accepted; ADR-012/013 added. Implementation plan with four phases added.
