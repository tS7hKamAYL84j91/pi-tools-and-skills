...
| `extensions/pi-panopticon/teams/team-handlers.ts` | 463 | Accept for now. It centralizes team tool handlers; split by command family only when a handler family grows independently. |
...
| `extensions/pi-panopticon/teams/team-form.ts` | 444 | Accept with caution. It mixes interactive authoring flow and file writes; future persistence or validation changes should extract stable helpers. |
| `extensions/pi-panopticon/spawner/spawner.ts` | 441 | Accept with caution. It coordinates spawn/RPC tool registration and process handoff; extract prompt-file handling or RPC control only when materially changed. |
...
| `extensions/pi-panopticon/teams/state.ts` | 435 | Accept with caution. It owns run-state serialization and migration; future schema or persistence changes should extract versioned codecs. |
...
| `extensions/pi-panopticon/registry/registry.ts` | 409 | Accept with caution. It owns registry lifecycle/persistence; future sync persistence work should extract shared write helpers rather than expanding this file. |
