...
- Preserved Automations-specific error wording and external workspace authorization.
...
- Focused Automations security, consumer-symlink, property, unit, and architecture tests: PASS (83 tests in final focused run). The symlink cases were baseline-green; the new RED→GREEN regression is the non-regular `EISDIR` hardening case.
...
- `extensions/pi-automations/store.ts`
- `tests/automations/pi-automations-store-security.test.ts`
