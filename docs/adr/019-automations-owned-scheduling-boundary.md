# ADR 019: Automations owns recurring scheduling over kanban board tools
...
`pi-kanban` exposes a reusable event-sourced board, board tools, snapshots, task files, a live overlay, and board-change watcher notifications. `pi-automations` owns operational scheduling through its file-backed schedule registry and pi-hosted internal scheduler.
...
- `pi-kanban` must not own cron, recurring schedules, morning briefs, operational review cadence, state-capture cadence, or Automations business policy.
- `pi-automations` owns recurring operational schedules and policies. Its scheduler may call or instruct use of `kanban_*` tools through scheduled prompts or a future narrow adapter, but ownership remains in Automations.
...
- Existing `pi-kanban` WIP limit and claim selection remain board mechanics; recurring WIP pick/capacity-review policy belongs in `pi-automations` schedules.
- `pi-automations` can coordinate kanban operations without `pi-kanban` depending on Automations.
...
- Import `pi-kanban` internals directly from `pi-automations`: rejected because current architecture enforces extension isolation and prevents cross-extension coupling.
