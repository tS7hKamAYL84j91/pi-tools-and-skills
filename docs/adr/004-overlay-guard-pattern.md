...
- `deleteTask` works for every column except `in-progress`
...
1. **Delete (`d`)**: Enter confirm-delete for tasks in `backlog`, `todo`, `blocked`, or `done`. For `in-progress` tasks, set status message "Delete unavailable: complete the task first".
