# ADR-038: Automations filesystem symlink confinement
...
T-795 identifies a path-traversal gap in Automations filesystem helpers. `assertInside()` is lexical and does not prevent an allowed path from traversing a persisted symlink to a location outside its intended root. `assertNoSymlinkComponents(root, target)` exists but the primary file-operation helpers do not consistently invoke it.
...
Explicitly selected external workspaces remain supported only when they contain `.pi/automations/workspace.env`; the remediation must preserve that bounded compatibility without granting arbitrary filesystem access.
...
| --- | --- | --- |
| Automations home runtime | `config.automationsHome` | Root itself must exist and not be a symlink. |
| Schedules/logs/locks | respective sub-root under `config.automationsHome` | Validate every component from `config.automationsHome` to the sub-root. |
| Managed workspaces | `workspaceRoot(config)` | Validate every component from `config.automationsHome` to the workspace root. |
...
- Symlink escapes are rejected consistently across Automations operations.
...
- Automations-home root-chain and external-workspace authorization/compatibility coverage.
