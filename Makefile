...
setup-package: ## Register one user-installable package globally (PACKAGE=pi-goal|pi-matrix|pi-panopticon|pi-research-tools|pi-teams)
	@test -n "$${PACKAGE:-}" || { echo "Usage: make setup-package PACKAGE=pi-goal|pi-matrix|pi-panopticon|pi-research-tools|pi-teams"; exit 2; }
...
setup-package-clean: ## Remove one user-installable package registration (PACKAGE=pi-goal|pi-matrix|pi-panopticon|pi-research-tools|pi-teams)
	@test -n "$${PACKAGE:-}" || { echo "Usage: make setup-package-clean PACKAGE=pi-goal|pi-matrix|pi-panopticon|pi-research-tools|pi-teams"; exit 2; }
