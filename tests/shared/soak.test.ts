...
const DETERMINISTIC_FAILURE_INTERVAL = 20;
 ...
 */
function createMockAgent(name: string, index = 0): AgentRecord {
	const now = Date.now();
	return {
		id: `soak-${name}-${now}`,
		name,
...
function simulateSendMessage(
	...
	const deliveryLatency = (sequence * 17) % 100;
	const success = sequence % DETERMINISTIC_FAILURE_INTERVAL !== 0;
 ...
 */
function verifyRegistryIntegrity(agents: AgentRecord[]): {
	valid: boolean;
	errors: string[];
} {
	const errors: string[] = [];
...
describe("Soak Test: Multi-Agent Messaging", () => {
	...
	afterAll(() => {
		...
		metrics.deliveryRate =
			messages.length > 0
				? (metrics.messagesReceived / messages.length) * 100
				: 0;
		...
		console.log(
			`  Messages received:     ${metrics.messagesReceived}/${messages.length}`,
		);
		console.log(`  Delivery rate:         ${metrics.deliveryRate.toFixed(1)}%`);
		console.log(
			`  Latency (avg/min/max): ${metrics.avgLatencyMs.toFixed(1)}ms / ${metrics.minLatencyMs.toFixed(1)}ms / ${metrics.maxLatencyMs.toFixed(1)}ms`,
		);
		console.log(`  Failed messages:       ${metrics.messagesFailed}`);
		console.log(
			`  Registry integrity:    ${metrics.registryIntegrity ? "✅ PASS" : "❌ FAIL"}`,
		);
	...
	it(`sends ${NUM_MESSAGES} messages across agents`, () => {
		...
		for (let batch = 0; batch < NUM_MESSAGES / MESSAGE_BATCH_SIZE; batch++) {
			for (let i = 0; i < MESSAGE_BATCH_SIZE; i++) {
				...
				const fromIdx = sequence % agents.length;
				const toIdx = (fromIdx + 1 + (batch % (agents.length - 1))) % agents.length;
				const from = agents[fromIdx];
				const to = agents[toIdx];

				if (from && to) {
					simulateSendMessage(from, to, messages, sequence);
				}
				sent++;
			}
		}
...
async function runSoakTest(): Promise<void> {
	...
	console.log(
		`  Delivered: ${delivered}/${messages.length} (${((delivered / messages.length) * 100).toFixed(1)}%)`,
	);
