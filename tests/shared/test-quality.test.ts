...
function isProductionEntry(path: string): boolean {
	...
	return path.startsWith(daemonSrcPrefix) && !path.slice(daemonSrcPrefix.length).includes(sep);
}
...
function testOnlyProductionModules(): string[] {
	...
	for (const file of IMPORTER_SCAN_ROOTS.flatMap((root) => listFiles(root, ".ts"))) {
	...
	for (const file of PRODUCTION_MODULE_ROOTS.flatMap((root) => listFiles(root, ".ts"))) {
		const path = relative(process.cwd(), file);
		...
		const importers = [...(importersByModule.get(resolve(file)) ?? [])]
			...
			.sort();
		const productionReachable = importers.some((importer) => !isTestFile(importer));
		if (productionReachable) {
			continue;
		}
		const detail = importers.length === 0 ? "zero importers" : `test-only importers: ${importers.join(", ")}`;
