...
describe("swarm command compatibility", () => {
	it("executes by default and supports explicit dry-run preflight", async () => {
		...
		registerSwarmCommand(
			{
				registerCommand(name: string, value: Omit<RegisteredCommand, "name">) {
					command = { name, ...value };
				},
		...
		await command.handler("inspect API --profile fast --dry-run", ctx);
		expect(notify).toHaveBeenLastCalledWith(
			expect.stringContaining("Swarm dry run; no workers spawned."),
			"info",
		);
