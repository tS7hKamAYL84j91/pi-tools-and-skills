...
vi.mock(
	"../../extensions/pi-panopticon/registry/external-registrar.js",
	() => ({
...
function makeCommands(): {
	api: ReturnType<typeof makeMockExtensionApi>;
	commands: Map<string, CommandDefinition>;
} {
	const api = makeMockExtensionApi();
	return {
		api,
		commands: api.registeredCommands as Map<string, CommandDefinition>,
	};
}
...
describe("panopticon UI slash commands", () => {
	...
	it("reports usage for missing external-agent arguments", async () => {
		...
		expect(notify).toHaveBeenNthCalledWith(
			1,
			"Usage: /agent-external-register <name>",
			"warning",
		);
		expect(notify).toHaveBeenNthCalledWith(
			2,
			"Usage: /agent-external-remove <name>",
			"warning",
		);
	...
	it("handles registration errors and case-insensitive removal", async () => {
		...
		await commands
			.get("agent-external-register")
			?.handler("worker", fixture.ctx);
		expect(fixture.notify).toHaveBeenCalledWith("mailbox unavailable", "error");

		registrarMocks.list.mockResolvedValue([
			{
				id: "worker-id",
				name: "Worker",
				kind: "external",
	...
	it("lists empty and populated external-agent registries", async () => {
		...
		registrarMocks.list
			...
			.mockResolvedValueOnce([
				{ id: "w", name: "worker", kind: "external", mailboxPath: "/mailbox" },
			]);
		...
		expect(fixture.notify).toHaveBeenCalledWith(
			expect.stringContaining("worker: /mailbox"),
			"info",
		);
	});
	...
	it("validates and applies agent list modes from arguments", async () => {
		...
		expect(fixture.notify).toHaveBeenCalledWith(
			expect.stringContaining("Invalid agent list mode"),
			"warning",
		);
		await commands.get("agents-mode")?.handler(" roots ", fixture.ctx);
		expect(deps.listMode.get(deps.registry.getRecord())).toBe("roots");
		expect(commands.get("agent-list-mode")?.description).toContain(
			"all|children|roots|scope",
		);
	});
