...
function ops(replies: readonly string[]): WireOp[] {
	return replies.map(
		(line) => JSON.parse(line) as Record<string, unknown> & { op: string },
	);
}
...
describe("registry sync wire codec", () => {
	it("round-trips messages and rejects empty, malformed, unknown-op, and oversized lines", () => {
		...
		expect(
			parseWireMessage(`{"op":"hello","agentId":"${"x".repeat(70_000)}"}`),
		).toBeUndefined();
	});
});
...
describe("daemon-side sync handler (design doc section 7)", () => {
	it("handshakes hello -> challenge -> proof -> hello_ok -> subscribe -> snapshot + events", async () => {
		...
		try {
			...
			session.onLine(
				encodeWireMessage({ op: "hello", agentId: setup.agentId }),
			);
			...
			await vi.waitFor(() => {
				expect(
					ops(capture.replies).some((reply) => reply.op === "snapshot"),
				).toBe(true);
			});
			const snapshot = ops(capture.replies).find(
				(reply) => reply.op === "snapshot",
			) as { seq: number; entries: Array<Record<string, unknown>> } | undefined;
			...
			const second = await setup.registry.registerAgent({
				displayName: "second",
				parentId: null,
				visibility: "global",
				scope: "task",
			...
			await vi.waitFor(() => {
				const events = ops(capture.replies).filter(
					(reply) => reply.op === "event",
				);
				expect(events.some((event) => event.agentId === second.agentId)).toBe(
					true,
				);
			});
	...
	it("rejects a wrong capability proof fail-closed", async () => {
		...
		try {
			...
			const wrongProof = capabilityProof(
				"not-the-secret",
			...
			session.onLine(
				encodeWireMessage({
					op: "hello_proof",
					agentId: setup.agentId,
					proof: wrongProof.toString("base64"),
				}),
			);
	...
	it("rejects subscribe-before-auth and daemon ops from the client side", async () => {
		...
		try {
			...
			const strict = acceptRegistrySyncConnection(
				{ registry: setup.registry },
				strictCapture.connection,
			);
