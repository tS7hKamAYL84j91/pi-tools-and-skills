...
function collector(): {
	...
	sink: (event: Record<string, unknown>) => Promise<void>;
} {
	const events: Record<string, unknown>[] = [];
	return {
		events,
		sink: async (event: Record<string, unknown>): Promise<void> => {
			events.push(event);
		},
	};
}

const ROOT_SPAWN: SpawnRequest = {
	displayName: "worker",
	parentId: null,
...
function assertAdmitted(
	result: Awaited<ReturnType<DaemonRegistry["admit"]>>,
): Extract<typeof result, { admitted: true }> {
	if (result.admitted !== true)
		throw new Error("expected admission, got rejection");
	return result;
}
...
describe("spawn persistence (ADR-0018 implementation path item 3)", () => {
	it("mints a stable opaque agent_id, admits a capability-bound instance, and persists durably", async () => {
		...
		try {
			...
			expect(registry.guardInputsFor(identity.agentId)).toEqual({
				parentId: null,
				visibility: "workspace",
				scope: "root",
			});
...
describe("M3 one-live-binding (latest authenticated admission wins)", () => {
	it("supersedes the previous instance with an audited notice and abort hook, no fan-out", async () => {
		...
		try {
			...
			expect(registry.bindingFor(identity.agentId)?.instanceId).toBe(
				second.instanceId,
			);
			expect(registry.capabilityFor(identity.agentId)?.capabilitySecret).toBe(
				second.capabilitySecret,
			);

			const invalidations = audit.events.filter(
				(event) => event.kind === "instance_invalidated",
			);
	...
	it("superseding one agent never touches another agent's binding", async () => {
		...
		try {
			...
			expect(registry.bindingFor(b.agentId)?.instanceId).toBe(
				bFirst.instanceId,
			);
			expect(registry.bindingFor(b.agentId)?.generation).toBe(
				bFirst.generation,
			);
		} finally {
			await rm(roots.stateRoot, { recursive: true, force: true });
	...
	it("refuses admission for an unregistered agent (fail-closed)", async () => {
		...
		try {
			const keys = await makeKeys(roots);
			const registry = new DaemonRegistry(roots, keys, {});
			await expect(registry.admit("a-unknown")).rejects.toThrow(
				/register before admitting/,
			);
		} finally {
			await rm(roots.stateRoot, { recursive: true, force: true });
		}
...
describe("M2 supervision: crash re-admission as a new generation", () => {
	it("a crash invalidates the binding and arms bounded respawn backoff; re-admission bumps generation", async () => {
		...
		try {
			...
			expect(
				audit.events.filter((event) => event.kind === "instance_crashed"),
			).toHaveLength(1);
			expect(
				audit.events.filter((event) => event.kind === "admission_rejected"),
			).toHaveLength(1);
		} finally {
			await rm(roots.stateRoot, { recursive: true, force: true });
		}
	...
	it("escalates 1s→2s→…→60s cap across rapid crash loops, and resets on stability", async () => {
		...
		try {
			...
			for (const expectedBackoff of expectedLadder) {
				const instanceId =
					registry.bindingFor(agentId)?.instanceId ?? "missing";
				const crash = await registry.noteInstanceCrash(agentId, instanceId);
			...
			const stableCrash = await registry.noteInstanceCrash(
				agentId,
				registry.bindingFor(agentId)?.instanceId ?? "missing",
			);
	...
	it("a crash notice for an already-superseded instance is a no-op", async () => {
		...
		try {
			...
			const stale = await registry.noteInstanceCrash(
				identity.agentId,
				first.instanceId,
			);
...
describe("M6 registry handoff: sequenced snapshot and change events", () => {
	...
	it("a concurrent snapshot serializes behind an in-flight admission (single-lock atomic read)", async () => {
		...
		try {
			...
			const entry = concurrent.entries.find(
				(candidate) => candidate.agentId === identity.agentId,
			);
...
describe("M6 client overlap rules (design doc section 7, formal review F7)", () => {
	...
	): RegistryEvent => ({ seq, at: "t", kind, agentId: "a-1" });
	...
	it("buffers events until the snapshot, drops seq <= snapshot, applies in order after", () => {
		...
		const reconciled = buffer.applySnapshot({
			seq: 5,
			generatedAt: "t",
			entries: [],
		});
		...
		expect(buffer.applyEvent(mkEvent(7, "instance_invalidated"))).toBe(
			"applied",
		);
		...
		expect(
			buffer.applySnapshot({ seq: 9, generatedAt: "t", entries: [] }).dropped,
		).toBe(0);
		expect(buffer.resyncRequired).toBe(false);
	});
...
describe("recovery: generation continuity across restarts (ADR section 7)", () => {
	it("rebuilds identities, invalidates stale live bindings, keeps generations", async () => {
		...
		try {
			...
			const entry = (await after.snapshot()).entries.find(
				(candidate) => candidate.agentId === identity.agentId,
			);
			...
			expect(
				after
					...
					.some(
						(event) =>
							event.kind === "instance_invalidated" &&
							event.reason === "restart_stale",
					),
			).toBe(true);
	...
	it("replaying recovery is a no-op (idempotent)", async () => {
		...
		try {
			...
			const entry = (await third.snapshot()).entries.find(
				(candidate) => candidate.agentId === identity.agentId,
			);
			expect(entry?.generation).toBe(2);
		} finally {
	...
	it("a tampered identity record is quarantined, never trusted, never dropped silently", async () => {
		...
		try {
			...
			const quarantine = await readdir(
				join(roots.stateRoot, "queue", "quarantine"),
			);
...
describe("M2 pi-close drain (bounded grace, then abort)", () => {
	...
	it("admissions during the drain grace proceed and are not captured by the deadline", async () => {
		...
		try {
			...
			await new Promise((resolve) => setTimeout(resolve, 5));
			const late = await registry.registerAgent({
				...ROOT_SPAWN,
			...
			expect(registry.bindingFor(late.agentId)?.instanceId).toBe(
				lateAdmission.instanceId,
			);
		} finally {
			await rm(roots.runtimeRoot, { recursive: true, force: true });
			await rm(roots.stateRoot, { recursive: true, force: true });
