...
describe("T-873 step 1 execution (dry-run/claim-check rollout)", () => {
	it("runs the 7-step work-order against fixture state roots with zero deliveries", async () => {
		...
		try {
			...
			results.step4 = {
				alreadyClaimed: repeat.filter(
					(decision) => decision.skippedReason === "already_claimed",
				).length,
			};
			...
			const rotate = await tick(
				roots,
				schedulesDir,
				"claim_check_only",
				new Date(2026, 0, 5, 9, 15),
				deliveries,
			);
			const quarterClaim = await readClaim(roots, "quarter-hourly");
			results.step5 = {
				quarterClaimRotated: quarterClaim?.minuteKey === "2026-01-05T09:15",
				deliveries: deliveries.length,
			};
			expect(
				rotate.find((decision) => decision.taskId === "quarter-hourly")?.due,
			).toBe(true);
