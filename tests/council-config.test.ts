...
type RegisteredHandler = (event: Record<string, unknown>, ctx: ExtensionContext) => Promise<unknown> | unknown;

const CONFIG_PATH = join(process.cwd(), "extensions", "pi-llm-council", "config.json");
...
function createFakeApi(): { api: ExtensionAPI; tools: Map<string, RegisteredTool>; handlers: Map<string, RegisteredHandler> } {
	const tools = new Map<string, RegisteredTool>();
	const handlers = new Map<string, RegisteredHandler>();
	const api = {
		registerTool(tool: RegisteredTool) { tools.set(tool.name, tool); },
		registerCommand(_name: string, _definition: unknown) { /* no-op */ },
...
describe("visible council config", () => {
	it("loads default council, chairman, and pair from extension config.json", () => {
	...
	it("forms the configured default pair on session_start", async () => {
		...
		const result = await pairList.execute("test", {}, undefined, undefined, contextFor(visible.defaultCouncil.members));
