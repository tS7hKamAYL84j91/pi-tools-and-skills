...
describe("boost_fusion authorization and fixed policy", () => {
	it("denies agents by default and permits a Principal bounded invocation", async () => {
		...
		const result = await principal.execute(
			"id",
			{
				prompt: "x",
				models: ["a/one", "b/two"],
	...
	it("allows pre-granted agents but rejects caller attempts to alter fixed caps", async () => {
		...
		await expect(
			tool.execute(
				"id",
				{ prompt: "x", panelSize: 4 },
				new AbortController().signal,
				undefined,
				ctx,
			),
		).rejects.toThrow(/fixed by operator/);
	...
	it("defaults to a single-model lease with no judge synthesis", async () => {
		const cwd = await mkdtemp(join(tmpdir(), "boost-fusion-single-"));
		roots.push(cwd);
		const globalPath = join(cwd, "global.json");
		await saveBoostSettings(
			"global",
		...
		// Rut-breaker default: one boost model, raw answer, no judge synthesis.
		expect(result.content[0]?.text).toBe("panel:a/one");
		const nodes = result.details?.nodes as ReadonlyArray<{
			role: string;
			model: string;
		}>;
