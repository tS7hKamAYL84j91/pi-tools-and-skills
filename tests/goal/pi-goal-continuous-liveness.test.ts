...
import { readGoalWatchdogConfig, startGoalWatchdog } from "../../extensions/pi-goal/goal-watchdog.js";
...
describe("pi-goal continuous liveness recovery", () => {
	it("warns once, does not nudge an active turn, then nudges one idle turn", async () => {
		...
		const started = startRun(await createTextGoal(cwd, "watch me"), 4, "continuous");
		...
		expect(persistedBeforeWatchdog?.lastProgressAt).toBe(new Date(0).toISOString());
		let active = true;
		let warnings = 0;
		let nudges = 0;
	...
	it("does not nudge while an in-process continuation is queued", async () => {
		...
		directories.push(cwd);
		const started = startRun(await createTextGoal(cwd, "queued"), 2, "continuous");
		await saveGoal(cwd, updateGoal(started, { lastProgressAt: new Date(0).toISOString() }));
		const scheduled: Array<() => void> = [];
		let nudges = 0;
		const stop = startGoalWatchdog({
	...
	it("caps operator liveness thresholds", () => {
		expect(readGoalWatchdogConfig({ PI_GOAL_LIVENESS_SOFT_MS: "1", PI_GOAL_LIVENESS_HARD_MS: "999999999999" })).toEqual({
			softTimeoutMs: 1_000,
	...
	it("pauses at the hard threshold and clears its timer", async () => {
		...
		directories.push(cwd);
		const started = startRun(await createTextGoal(cwd, "hard stop"), 2, "continuous");
		await saveGoal(cwd, updateGoal(started, { lastProgressAt: new Date(0).toISOString() }));
		expect((await loadGoal(cwd))?.lastProgressAt).toBe(new Date(0).toISOString());
		let errors = 0;
		const scheduled: Array<() => void> = [];
		const stop = startGoalWatchdog({
			cwd,
