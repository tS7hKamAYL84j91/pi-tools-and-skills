/** Regression tests for model-agnostic pi-automations schedule delivery. */
import { mkdir, readFile, rm, writeFile } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { PiScheduler } from "../../extensions/pi-automations/pi-scheduler.js";
import { addSchedule, listSchedules } from "../../extensions/pi-automations/schedules.js";
...
async function writeLegacyPinnedSchedule(automationsHome: string, taskId: string): Promise<void> {
...
describe("PiScheduler model-agnostic delivery", () => {
	...
	it("runs a legacy model-pinned schedule without subscribing to model changes", async () => {
		const automationsHome = join(tmpdir(), `pi-automations-model-agnostic-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeLegacyPinnedSchedule(automationsHome, "daily");
	...
	it("continues delivering the same schedule across trigger dates independent of session model", async () => {
		const automationsHome = join(tmpdir(), `pi-automations-model-change-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeLegacyPinnedSchedule(automationsHome, "weekly");
			await scheduler.reconcile({ automationsHome });
			await scheduler.tick(new Date("2026-01-05T09:00:00"));
			await scheduler.flush();
	...
	it("does not write model identity into newly created schedules", async () => {
		const automationsHome = join(tmpdir(), `pi-automations-model-persist-${process.pid}-${Date.now()}`);
		try {
			const created = await addSchedule({ automationsHome }, {
				room: "general",
				name: "agnostic",
				cron: "0 9 * * 1",
				prompt: "Work.",
			...
			const env = await readFile(join(automationsHome, "schedules", `${created.taskId}.env`), "utf8");
			expect(env).not.toContain("MODEL_SNAPSHOT");
			const loaded = await listSchedules({ automationsHome });
