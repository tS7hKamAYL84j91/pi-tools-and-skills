...
import { commandSummary, widgetLines } from "../../extensions/pi-automations/format.js";
...
describe("commandSummary", () => {
	it("formats successful command output", () => {
		const summary = commandSummary("automations-status", { code: 0, stdout: "ok\n", stderr: "" });
		expect(summary).toBe("automations-status exit=0\n\nok");
	});

	it("formats failed command stderr", () => {
		const summary = commandSummary("automations-doctor", { code: 1, stdout: "", stderr: "bad\n" });
		expect(summary).toBe("automations-doctor exit=1\n\nbad");
	});

	it("truncates long command output", () => {
		const summary = commandSummary("automations-doctor", { code: 0, stdout: `${"line\n".repeat(2001)}`, stderr: "" });
