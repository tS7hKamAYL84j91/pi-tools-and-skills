/** Automations schedules + internal scheduler unit tests. */
...
import {
	...
	scheduleMatchesDate,
} from "../../extensions/pi-automations/pi-scheduler.js";
import {
	addSchedule,
	validateCronExpr,
	formatScheduleList,
	renderInternalSchedulePlan,
} from "../../extensions/pi-automations/schedules.js";
...
describe("schedules", () => {
	...
	describe("PiScheduler", () => {
		...
		it("clears runtime state on stop", async () => {
			...
			scheduler.start({ automationsHome: join(tmpdir(), "missing-automations-home") });
		...
		it("records queued telemetry on successful sendUserMessage", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-queued-${process.pid}-${Date.now()}`,
			);
		...
		it("records failed telemetry when sendUserMessage throws", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-failed-${process.pid}-${Date.now()}`,
			);
			const schedulesDir = join(automationsHome, "schedules");
			const promptPath = join(schedulesDir, "daily.prompt");
			await mkdir(schedulesDir, { recursive: true });
		...
		it("resets telemetry counters and timestamps on stop", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-reset-${process.pid}-${Date.now()}`,
			);
			const schedulesDir = join(automationsHome, "schedules");
			const promptPath = join(schedulesDir, "daily.prompt");
			await mkdir(schedulesDir, { recursive: true });
		...
		it("records reconcile errors instead of silently hiding them", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-bad-schedule-${process.pid}-${Date.now()}`,
			);
			const schedulesDir = join(automationsHome, "schedules");
			await mkdir(schedulesDir, { recursive: true });
			await writeFile(
		...
		it("records malformed schedule expressions during ticks", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-invalid-field-${process.pid}-${Date.now()}`,
			);
			const schedulesDir = join(automationsHome, "schedules");
	...
	describe("addSchedule", () => {
		it("rejects invalid cron fields before writing schedule files", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-invalid-add-${process.pid}-${Date.now()}`,
			);
			try {
				await expect(
					addSchedule(
	...
	describe("renderInternalSchedulePlan", () => {
		it("previews enabled schedules without prompt text", async () => {
			const automationsHome = join(
				tmpdir(),
				`pi-automations-preview-${process.pid}-${Date.now()}`,
			);
			try {
				await addSchedule(
					{ automationsHome },
				...
				const result = await renderInternalSchedulePlan({ automationsHome });
