/**
 * Regression tests for pi-automations resumable schedule continuation.
 */
...
async function writeSchedule(automationsHome: string, taskId: string, workspaceId: string, continuation = false): Promise<void> {
...
describe("PiScheduler continuation", () => {
	...
	it("carries prior-run summary across two consecutive triggers", async () => {
		...
		const automationsHome = join(tmpdir(), `pi-automations-cont-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
	...
	it("does not inject phantom summary when no completed run exists", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-no-prior-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
	...
	it("keeps only one run-state file per task after multiple triggers", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-shape-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
			...
			const runStatePath = join(automationsHome, "schedule-runs", "daily.json");
	...
	it("keeps injected continuation summary length constant across repeated triggers", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-bloat-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
	...
	it("does not inject stale prior-run summary", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-stale-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
			await mkdir(join(automationsHome, "schedule-runs"), { recursive: true });
			await writeFile(
				join(automationsHome, "schedule-runs", "daily.json"),
				JSON.stringify({
					taskId: "daily",
					runId: "run-old",
					status: "complete",
	...
	it("persists active runs as interrupted before stop resets scheduler state", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-stop-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
	...
	it("does not inject interrupted run summary", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-interrupt-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
	...
	it("non-continuation schedules remain stateless", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-stateless-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", false);
			await scheduler.reconcile({ automationsHome });
			await scheduler.tick(new Date("2026-01-05T09:00:00"));
			await scheduler.flush();
			const prompt = pi.sent[0]?.message ?? "";
			...
			expect(existsSync(join(automationsHome, "schedule-runs", "daily.json"))).toBe(false);
		} finally {
			await scheduler.stop();
			await rm(automationsHome, { recursive: true, force: true });
		}
	...
	it("removeSchedule deletes run-state file", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "admin-assistant";
		const automationsHome = join(tmpdir(), `pi-automations-remove-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
			...
			const { removeSchedule } = await import("../../extensions/pi-automations/schedules.js");
			await removeSchedule({ automationsHome }, "daily");
			expect(existsSync(join(automationsHome, "schedule-runs", "daily.json"))).toBe(false);
		} finally {
			await scheduler.stop();
			await rm(automationsHome, { recursive: true, force: true });
	...
	it("still drops continuation schedule on workspace mismatch", async () => {
		process.env[AUTOMATIONS_WORKSPACE_ID_ENV] = "wrong-workspace";
		const automationsHome = join(tmpdir(), `pi-automations-cont-guard-${process.pid}-${Date.now()}`);
		const pi = makePi();
		const scheduler = new PiScheduler(pi as never);
		try {
			await writeSchedule(automationsHome, "daily", "admin-assistant", true);
