...
import { listApprovalArtifacts } from "../../extensions/pi-automations/approval-inbox.js";
import { appendScheduleLog } from "../../extensions/pi-automations/scheduler-log.js";
import { loadRunState } from "../../extensions/pi-automations/scheduler-run-state.js";
import { listSchedules, removeSchedule } from "../../extensions/pi-automations/schedules.js";
import { automationsStatus } from "../../extensions/pi-automations/status.js";
import { readWorkspaceContext } from "../../extensions/pi-automations/workspace-context.js";

const roots: string[] = [];

async function createRoot(label: string): Promise<string> {
	const root = join(tmpdir(), `pi-automations-consumer-${label}-${process.pid}-${Date.now()}-${roots.length}`);
	roots.push(root);
...
describe("Automations production consumer symlink confinement", () => {
	it("rejects symlinked schedule entries and prompt targets", async () => {
		...
		await expect(listSchedules({ automationsHome: home })).rejects.toThrow(/symlinked Automations directory entry/);
	...
	it("validates all schedule removals before mutating any file", async () => {
		...
		await expect(removeSchedule({ automationsHome: home }, "daily")).rejects.toThrow(/symlinked Automations path component/);
	...
	it("rejects symlinked workspace and approval entries through status and inbox consumers", async () => {
		...
		await expect(automationsStatus({ automationsHome: home })).rejects.toThrow(/symlinked Automations directory entry/);
		...
		await expect(listApprovalArtifacts({ automationsHome: home })).rejects.toThrow(/symlinked Automations directory entry/);
	});
	...
	it("rejects external workspace context symlinks after metadata authorization", async () => {
		...
		await expect(readWorkspaceContext({ automationsHome: home }, workspace, workspace)).rejects.toThrow(/symlinked Automations path component/);
	});
	...
	it("rejects symlinked run-state and schedule-log roots", async () => {
		...
		await expect(loadRunState({ automationsHome: home }, "daily")).rejects.toThrow(/symlinked Automations path component/);

		await mkdir(join(home, "logs"));
		await symlink(outside, join(home, "logs", "schedules"));
		await expect(appendScheduleLog({ automationsHome: home }, "daily", "QUEUED")).rejects.toThrow(/symlinked Automations path component/);
	});
});
