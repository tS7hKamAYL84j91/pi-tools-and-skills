...
describe("bounded Automations path properties", () => {
	it("accepts generated safe nested paths and rejects generated escapes", () => {
		const root = resolve("/fast-check/automations-root");
		assertProperty(
			fc.property(
	...
	describe("ConfinedStore non-absolute targets", () => {
		...
		beforeAll(async () => {
			root = await mkdtemp(join(tmpdir(), "pi-automations-fast-check-"));
			store = await ConfinedStore.forAutomationsHome({ automationsHome: root });
		});
			...
			await assertAsyncProperty(
				fc.asyncProperty(relativePathArbitrary, async (relativePath) => {
					...
					await expect(store.fileExists(relativePath)).rejects.toThrow(
						/Automations target must be absolute/,
					);
				}),
			);
		});
