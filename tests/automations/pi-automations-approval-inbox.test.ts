...
import { minuteKey } from "../../extensions/pi-automations/scheduler-util.js";
import { tmpdir } from "node:os";
import { PiScheduler } from "../../extensions/pi-automations/pi-scheduler.js";
import { approveApproval, listApprovalArtifacts, parkApproval, readApprovalArtifact, rejectApproval } from "../../extensions/pi-automations/approval-inbox.js";
import { ConfinedStore } from "../../extensions/pi-automations/store.js";
import { registerAutomationsApprovalTools } from "../../extensions/pi-automations/tools-approval.js";
...
describe("Automations approval inbox", () => {
	it("parks a gated scheduled run durably and does not complete or deliver it", async () => {
		const home = join(tmpdir(), `pi-automations-approval-${process.pid}-${Date.now()}`);
		homes.push(home);
		...
		process.env.AUTOMATIONS_WORKSPACE_ID = "room-a";
		delete process.env[PANOPTICON_SCOPE_ENV];
		delete process.env[PANOPTICON_SPAWN_NAME_ENV];
		const calls: string[] = [];
		...
		await scheduler.reconcile({ automationsHome: home });
		await scheduler.tick(new Date("2026-01-05T09:00:00"));
			await scheduler.flush();
		const artifacts = await listApprovalArtifacts({ automationsHome: home });
		expect(artifacts).toHaveLength(1);
		const requestId = artifacts[0]?.requestId;
		if (!requestId) throw new Error("approval artifact was not created");
		const artifact = await readApprovalArtifact({ automationsHome: home }, requestId);
		...
		expect((await approveApproval({ automationsHome: home }, requestId)).status).toBe("approved");
		expect(await scheduler.resumeApprovedRun({ automationsHome: home }, requestId)).toBe(true);
		...
		expect((await readApprovalArtifact({ automationsHome: home }, requestId))?.status).toBe("completed");
	...
	it("marks an ambiguous approval-resumed send uncertain without replay", async () => {
		const home = join(tmpdir(), `pi-automations-approval-uncertain-${process.pid}-${Date.now()}`);
		homes.push(home);
		const schedules = join(home, "schedules");
		await mkdir(schedules, { recursive: true });
		const promptPath = join(schedules, "gated.prompt");
		...
		await scheduler.flush();
		const requestId = (await listApprovalArtifacts({ automationsHome: home }))[0]?.requestId;
		if (!requestId) throw new Error("approval artifact was not created");
		process.env.PI_PRINCIPAL = "1";
		await approveApproval({ automationsHome: home }, requestId);
		expect(await scheduler.resumeApprovedRun({ automationsHome: home }, requestId)).toBe(false);
		const slotPath = join(home, "schedule-runs", `gated.slot-${Buffer.from(minuteKey(new Date("2026-01-05T09:00:00Z"))).toString("base64url")}.json`);
	...
	it("returns a failed tool result when an approved run cannot resume", async () => {
		const project = join(tmpdir(), `pi-automations-approval-tool-${process.pid}-${Date.now()}`);
		homes.push(project);
		const home = join(project, ".pi", "automations");
		await mkdir(join(home, "workspace"), { recursive: true });
		await parkApproval({ config: { automationsHome: home }, taskId: "task", runId: "run", prompt: "prompt", requestId: "request" });
		process.env.PI_PRINCIPAL = "1";
		const tools = new Map<string, RegisteredApprovalTool>();
		const resumeApprovedRun = vi.fn(async () => false);
		registerAutomationsApprovalTools({
			registerTool(definition: RegisteredApprovalTool) {
				tools.set(definition.name, definition);
			},
		} as never, resumeApprovedRun);
		const approve = tools.get("automations_approval_approve");
		...
		expect((await readApprovalArtifact({ automationsHome: home }, "request"))?.status).toBe("approved");
	});

	it("keeps concurrent approve and reject results consistent with the final decision", async () => {
		const home = join(tmpdir(), `pi-automations-approval-decision-${process.pid}-${Date.now()}`);
		homes.push(home);
		await parkApproval({ config: { automationsHome: home }, taskId: "task", runId: "run", prompt: "prompt", requestId: "request" });
		process.env.PI_PRINCIPAL = "1";

		const results = await Promise.all([
			approveApproval({ automationsHome: home }, "request"),
			rejectApproval({ automationsHome: home }, "request"),
		]);
		const final = await readApprovalArtifact({ automationsHome: home }, "request");
	...
	it("serializes concurrent reject and approve decisions before dispatch", async () => {
		const project = join(tmpdir(), `pi-automations-approval-race-${process.pid}-${Date.now()}`);
		homes.push(project);
		const home = join(project, ".pi", "automations");
		await mkdir(join(home, "workspace"), { recursive: true });
		await parkApproval({ config: { automationsHome: home }, taskId: "task", runId: "run", prompt: "prompt", requestId: "request" });
		...
		const reject = tools.get("automations_approval_reject");
		const approve = tools.get("automations_approval_approve");
		...
		expect((await readApprovalArtifact({ automationsHome: home }, "request"))?.status).toBe("rejected");
		expect(resumeApprovedRun).not.toHaveBeenCalled();
	});

	it("requires principal authority for approval decisions", async () => {
		const home = join(tmpdir(), `pi-automations-approval-principal-${process.pid}-${Date.now()}`);
		homes.push(home);
		await mkdir(home, { recursive: true });
		await parkApproval({ config: { automationsHome: home }, taskId: "task", runId: "run", prompt: "prompt", requestId: "request" });
		await expect(approveApproval({ automationsHome: home }, "request")).rejects.toThrow(/principal authority/);
		process.env.PI_PRINCIPAL = "1";
		expect((await approveApproval({ automationsHome: home }, "request")).status).toBe("approved");
	});
});
