...
import { createScheduleSlotState } from "../../extensions/pi-automations/scheduler-slot-state.js";
import type { AutomationsConfig, ScheduleEntry } from "../../extensions/pi-automations/types.js";
...
async function setup(): Promise<{ config: AutomationsConfig; state: ReturnType<typeof createScheduleSlotState> }> {
	const home = await mkdtemp(join(tmpdir(), "automations-slot-"));
	homes.push(home);
	await mkdir(join(home, "schedule-runs"));
	const config = { automationsHome: home } as AutomationsConfig;
...
describe("ADR-060 scheduler slot admission", () => {
	...
	it("fails closed on malformed records instead of reclaiming them", async () => {
		...
		await writeFile(join(config.automationsHome, "schedule-runs", `task.slot-${encoded}.json`), "not-json\n");
