...
import {
	...
	preflightHierarchicalSwarm,
} from "./swarm-compat.js";
...
function compatibilityRuns(registration: SwarmRegistration) {
	return registration.teams.stateManager
		.list()
		.filter((run) => run.team === HIERARCHICAL_SWARM_TEAM_ID);
}
...
export function registerSwarmTools(
	...
	pi.registerTool({
		...
		label: "Run Swarm",
		description:
			"Compatibility alias for team_run hierarchical-swarm-default. Dry-run defaults to true.",
		...
		async execute(_id, params, _signal, _onUpdate, ctx) {
			if (params.wip !== undefined) {
				return fail(
					"swarm_run wip is unsupported by the compatibility alias; configure hierarchical-swarm bounds in the team manifest.",
					{ code: "validation" },
				);
			}
			...
			if (params.dry_run !== false) {
				try {
					const preflight = preflightHierarchicalSwarm({
						cwd: ctx.cwd,
						goal: params.goal,
						profile,
					});
					return ok(preflight.text, {
						team: preflight.team.id,
						dryRun: true,
						compatibility: "hierarchical-swarm",
					});
				} catch (error) {
			...
			const teamParams = {
				id: HIERARCHICAL_SWARM_TEAM_ID,
				prompt: params.goal,
				profile,
				...(params.async ? { async: true } : {}),
			};
			return params.async
				? registration.teams.runAsync(teamParams, ctx)
				: registration.teams.run(teamParams, ctx);
		},
	});
	...
	pi.registerTool({
		...
		async execute(_id, params) {
			...
			if (!run || run.team !== HIERARCHICAL_SWARM_TEAM_ID) {
				return fail(
					`No hierarchical-swarm team run "${params.swarmId}". Legacy swarm IDs are not available; use team_runs.`,
					{ code: "validation" },
				);
			}
			return ok(`${run.id} ${run.status} nodes=${run.nodes.length}`, {
				run,
				compatibility: "hierarchical-swarm",
			});
		},
	...
	pi.registerTool({
		...
		async execute(_id, params) {
			const runs = compatibilityRuns(registration).filter(
				(run) =>
					!params.activeOnly ||
					run.status === "pending" ||
					run.status === "running" ||
			...
			return ok(
				runs.length === 0
					? "No hierarchical-swarm team runs."
					: runs
							.map((run) => `${run.id} ${run.status} nodes=${run.nodes.length}`)
							.join("\n"),
	...
	pi.registerTool({
		...
		async execute(_id, params) {
			...
			if (!run || run.team !== HIERARCHICAL_SWARM_TEAM_ID) {
				return fail(
					`No hierarchical-swarm team run "${params.swarmId}". Legacy swarm IDs cannot be stopped.`,
					{ code: "validation" },
				);
			}
			const stopped = registration.teams.stateManager.requestStop(
				run.id,
				params.reason ?? "cancelled by swarm compatibility alias",
			);
			return stopped
				? ok(`Team run ${run.id} stopping.`, {
						runId: run.id,
						compatibility: "hierarchical-swarm",
					})
