/**
 * spawner-tools — Tool registrations for the spawner extension.
 */
...
export function registerSpawnAgentTool(pi: ExtensionAPI, ctx: SpawnerContext): void {
	pi.registerTool({
		name: "spawn_agent",
		label: "Spawn Agent",
		description:
		...
		async execute(_toolCallId, params, _signal): Promise<ToolResult> {
			...
			const existing = ctx.agents.get(params.name);
			...
			ctx.agents.set(params.name, agent);
...
export function registerRpcSendTool(pi: ExtensionAPI, ctx: SpawnerContext): void {
	pi.registerTool({
		name: "rpc_send",
		label: "RPC Send",
		description:
		...
		async execute(_toolCallId, params, _signal): Promise<ToolResult> {
			const startedAt = Date.now();
			const agent = ctx.agents.get(params.name);
			if (!agent) {
				return fail(
					`No spawned agent named "${params.name}". Known: ${[...ctx.agents.keys()].join(", ") || "(none)"}`,
					{
						error: "not_found",
...
export function registerListSpawnedTool(pi: ExtensionAPI, ctx: SpawnerContext): void {
	pi.registerTool({
		name: "list_spawned",
		label: "List Spawned",
		...
		async execute(_toolCallId, params, _signal): Promise<ToolResult> {
			...
			if (params.name) {
				...
				if (!agent) {
					return fail(
						`No agent named "${params.name}". Known: ${[...ctx.agents.keys()].join(", ")}`,
						{ error: "not_found" },
					);
				}
			...
			const rows = [...ctx.agents.values()].map((a) => {
			...
			return ok(
				`${ctx.agents.size} spawned agent(s):\n${rows.join("\n")}\n\nUse list_spawned with a name for details.`,
				{ count: ctx.agents.size, agents: [...ctx.agents.keys()] },
			);
		},
	});
}
...
export function registerKillAgentTool(pi: ExtensionAPI, ctx: SpawnerContext): void {
	pi.registerTool({
		name: "kill_agent",
		label: "Kill Agent",
		description:
