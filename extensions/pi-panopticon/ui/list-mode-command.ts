...
import { filterAgentList, isAgentListMode } from "../registry/visibility.js";

const MODE_ITEMS: SelectItem[] = [
	{
		value: "all",
		label: "all",
		description: "Show every agent allowed by visibility rules",
	},
	{
		value: "children",
		label: "children",
		description: "Show roots plus my own children; hide other agents' children",
	},
	{
		value: "roots",
		label: "roots",
		description: "Show root/manual agents plus direct family",
	},
	{
		value: "scope",
		label: "scope",
		description: "Show parent, siblings, and my children",
	},
];
...
function setAgentListMode(
	...
	const visibleCount = filterAgentList(
		self,
...
async function openAgentListModeOverlay(
	...
	const selected = await ctx.ui.custom<string | null>(
		(tui, theme, _kb, done) => {
			...
			container.addChild(
				new Text(
					theme.fg("accent", theme.bold(" Agent List Mode")) +
						theme.fg("dim", ` - current: ${current}`),
					1,
					0,
				),
			);
...
export function registerAgentListModeControls(
	...
	pi.registerCommand("agent-list-mode", {
		description:
			"Choose agent list/widget mode. Usage: /agent-list-mode [all|children|roots|scope]",
		handler: handleCommand,
	});

	pi.registerCommand("agents-mode", {
		description:
			"Set agent list/widget mode. Usage: /agents-mode [all|children|roots|scope]",
		handler: handleCommand,
	});
}
