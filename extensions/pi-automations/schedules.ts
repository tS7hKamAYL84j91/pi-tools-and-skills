/** Automations schedule registry; runtime execution is owned by the pi-hosted scheduler. */
...
import type { AutomationsConfig, CommandResult, ScheduleAddInput, ScheduleEntry } from "./types.js";
...
function scheduleEnvPath(config: AutomationsConfig, taskId: string): string {
	assertSafeId("task id", taskId);
	return join(scheduleRoot(config), `${taskId}.env`);
}

function schedulePromptPath(config: AutomationsConfig, taskId: string): string {
	assertSafeId("task id", taskId);
	return join(scheduleRoot(config), `${taskId}.prompt`);
}

function scheduleLogPath(config: AutomationsConfig, taskId: string): string {
	assertSafeId("task id", taskId);
	return join(scheduleLogRoot(config), `${taskId}.log`);
}

function scheduleRunsRoot(config: AutomationsConfig): string {
	return join(config.automationsHome, "schedule-runs");
}

export function scheduleRunsPath(config: AutomationsConfig, taskId: string): string {
	assertSafeId("task id", taskId);
	return join(scheduleRunsRoot(config), `${taskId}.json`);
}

async function parseSchedule(config: AutomationsConfig, store: ConfinedStore, envPath: string): Promise<ScheduleEntry> {
...
export async function listSchedules(config: AutomationsConfig): Promise<ScheduleEntry[]> {
...
export async function addSchedule(config: AutomationsConfig, input: ScheduleAddInput): Promise<ScheduleEntry> {
...
export async function removeSchedule(config: AutomationsConfig, taskId: string): Promise<string> {
	assertSafeId("task id", taskId);
	...
	return `automations-schedule: removed ${taskId}`;
}

async function readSchedule(config: AutomationsConfig, taskId: string): Promise<ScheduleEntry> {
	const store = await ConfinedStore.forScheduleRoot(config);
	const envPath = scheduleEnvPath(config, taskId);
...
async function logTask(config: AutomationsConfig, taskId: string, message: string): Promise<void> {
...
export async function runSchedule(config: AutomationsConfig, taskId: string, dryRun: boolean): Promise<CommandResult> {
	const schedule = await readSchedule(config, taskId);
	const sessionDir = join(config.automationsHome, "pi-sessions", "schedules", schedule.taskId);
	...
	if (!schedule.enabled) return { code: 0, stdout: "", stderr: `automations-schedule: task disabled: ${schedule.taskId}` };
	if (dryRun) {
		return {
			code: 0,
			stderr: "",
	...
	return { code: 1, stdout: "", stderr: "automations-schedule: direct execution is disabled; enabled schedules run through the pi-hosted pi-scheduler" };
}

export async function renderInternalSchedulePlan(config: AutomationsConfig): Promise<CommandResult> {
	...
	const body = schedules.length > 0
		...
		: "no enabled Automations schedules";
	return { code: 0, stderr: "", stdout: ["Automations pi-scheduler plan", "============================", body].join("\n") };
}
