...
export async function activeIdentity(pi: ExtensionAPI, config: AutomationsConfig): Promise<DeliveryIdentity> {
	...
	return {
		workspaceId: process.env.AUTOMATIONS_WORKSPACE_ID ?? await currentWorkspaceLabel(config, process.cwd()) ?? "",
