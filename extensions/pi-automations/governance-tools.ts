/**
 * Automations governance tool registrations.
 */
...
export function registerGovernanceTools(
	pi: ExtensionAPI,
	configFor: (ctx: ExtensionContext, cwd?: string) => Promise<AutomationsConfig>,
): void {
	pi.registerTool({
		name: "automations_governance_resolve",
		label: "Automations Governance Resolve",
		description:
			...
			try {
				...
				if (resolution.escalate) {
					...
					try {
						...
						await store.appendPrivateLog(join(config.automationsHome, "governance", "escalation.log"), `[${isoUtc()}] ${alert}\n`);
					}
				}
