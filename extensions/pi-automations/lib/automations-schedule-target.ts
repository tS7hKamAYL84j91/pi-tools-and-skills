...
export async function readScheduleTargetAgent(config: AutomationsConfig, taskId: string): Promise<string | undefined> {
	const store = await ConfinedStore.openRoot(config.automationsHome);
	if (!store) return undefined;
	const envPath = join(config.automationsHome, "schedules", `${taskId}.env`);
