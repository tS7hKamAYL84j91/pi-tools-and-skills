...
function scheduleRunMarker(taskId: string, runId: string): string {
	return `<!-- automations-scheduled-run taskId="${taskId}" runId="${runId}" -->`;
}

function extractScheduledRunMarker(text: string): { taskId: string; runId: string } | undefined {
	const match = text.match(/<!--\s*automations-scheduled-run\s+taskId="([^"]+)"\s+runId="([^"]+)"\s*-->/);
...
export function renderScheduledPrompt(schedule: ScheduleEntry, priorSummary?: PriorSummary): string {
	const continuationHeader = priorSummary
		...
		: "";
	return [
		`${continuationHeader}Automations scheduled task: ${schedule.taskName} (${schedule.taskId})`,
		`Workspace: ${schedule.workspaceId}`,
		`Room: ${schedule.roomId || "(none)"}`,
		"",
		"Run the following scheduled Automations prompt. Use Automations workspace tools when useful, and record durable non-secret outcomes with automations_workspace_update.",
		"",
