...
function slotPath(config: AutomationsConfig, taskId: string, slotKey: string): string {
	assertSafeId("task id", taskId);
	const encodedSlot = Buffer.from(slotKey).toString("base64url");
	return join(config.automationsHome, "schedule-runs", `${taskId}.slot-${encodedSlot}.json`);
}

async function storeFor(config: AutomationsConfig): Promise<ConfinedStore> {
	return await ConfinedStore.openAutomationsHome(config) ?? await ConfinedStore.createAutomationsHome(config);
}
...
async function hasNewerDeliveredSlot(store: ConfinedStore, config: AutomationsConfig, taskId: string, slotKey: string): Promise<boolean> {
	const root = join(config.automationsHome, "schedule-runs");
...
export function createScheduleSlotState(getConfig: () => AutomationsConfig | undefined): ScheduleSlotState {
	return {
...
async function claimScheduleSlot(config: AutomationsConfig, taskId: string, slotKey: string, now: Date): Promise<SlotClaim | undefined> {
	const store = await storeFor(config);
