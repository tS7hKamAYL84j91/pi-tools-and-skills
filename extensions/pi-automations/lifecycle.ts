/**
 * Automations extension lifecycle hooks.
 */
...
export function formatAutomationsStatusSlot(workspace?: string, scheduler?: SchedulerSnapshot): string {
	const scope = workspace ?? "on";
	if (!scheduler) return workspace ? `automations: ${scope}` : "automations: on ✓";
	...
	return `automations: ${scope} ${health}${scheduleText}${queueText}${failText}${droppedText}`;
}
...
async function updateStatus(ctx: ExtensionContext, scheduler: PiScheduler): Promise<void> {
	...
	if (!workspace && !await ConfinedStore.openAutomationsHome(config)) {
		ctx.ui.setStatus("automations", undefined);
		return;
	}
	ctx.ui.setStatus("automations", formatAutomationsStatusSlot(workspace, scheduler.snapshot()));
}
...
async function contextInstruction(ctx: ExtensionContext): Promise<string | undefined> {
	...
	return [
		"Automations workspace context is available for this session.",
		"Use automations_workspace_read before workspace-sensitive work when relevant.",
		"Use automations_workspace_update only for stable, useful, non-secret facts.",
	].join("\n");
}
