/** Automations workspace filesystem operations. */
...
import type { AutomationsConfig, CreateWorkspaceInput, WorkspaceSummary } from "./types.js";
...
export async function createWorkspace(
	...
	for (const path of [dir, join(dir, ".pi"), join(dir, ".pi", "automations"), join(dir, "logs"), join(dir, "tmp")]) {
	...
	if (!await store.fileExists(contextPath)) {
		await store.appendPrivateLog(contextPath, [
			`# Automations Workspace: ${workspaceId}`,
			"",
...
export function formatWorkspaceList(workspaces: WorkspaceSummary[]): string {
	if (workspaces.length === 0) return "No Automations workspaces found.";
