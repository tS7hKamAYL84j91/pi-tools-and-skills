/** Continuation run-state persistence for the pi-automations pi-scheduler. */

import { ConfinedStore } from "./store.js";
import { loadRunState as loadSharedRunState, saveRunState as saveSharedRunState } from "./lib/automations-run-state.js";
import { scheduleRunsPath } from "./schedules.js";
import { assertSafeId } from "./store-paths.js";
import type { ScheduleRunState } from "./lib/automations-run-state.js";
import type { AutomationsConfig, ScheduleEntry } from "./types.js";

export type { ScheduleRunState } from "./lib/automations-run-state.js";
...
export async function loadRunState(config: AutomationsConfig, taskId: string): Promise<ScheduleRunState | undefined> {
	try {
		...
		if (error instanceof Error) throw normalizeAutomationsStoreError(error);
		throw error;
	}
}
...
function normalizeAutomationsStoreError(error: Error): Error {
	if (error.message.includes("Refusing symlinked path component")) {
		return new Error(error.message.replace("Refusing symlinked path component", "Refusing symlinked Automations path component"));
	}
	if (error.message.includes("Refusing symlinked root")) {
		return new Error(error.message.replace("Refusing symlinked root", "Refusing symlinked Automations root"));
	}
	return error;
}
...
function runHistoryPath(config: AutomationsConfig, taskId: string): string {
...
export async function loadRunHistory(config: AutomationsConfig, taskId: string): Promise<ScheduleRunHistory | undefined> {
...
export async function countContinuationReady(config: AutomationsConfig, schedules: ScheduleEntry[]): Promise<number> {
	let ready = 0;
