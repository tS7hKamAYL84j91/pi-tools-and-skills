...
export function goalContextMessage(state: GoalState): string {
	...
	const run = state.runActive ? `\nCurrent ${state.runMode ?? "manual"} run: turn ${state.turnsUsed + 1} of ${state.turnBudget} (runId=${state.runId ?? "legacy"}, milestoneRevision=${state.milestoneRevision ?? 0})` : "";
	...
	const verification = state.lastVerification
		? `\nLast verification (untrusted report): exitCode=${state.lastVerification.exitCode} · ${escapeXml(state.lastVerification.outputSummary.slice(0, 200))}`
		: "";
