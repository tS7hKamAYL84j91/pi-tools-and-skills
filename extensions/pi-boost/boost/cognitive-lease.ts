/** Parallel CognitiveLease execution: panel selection, concurrent querying, and judge synthesis. */
...
/** Execute a bounded, single-turn CognitiveLease deliberation. */
...
export async function executeCognitiveLease(
	...
	const panelSize = result.nodes.filter((node) => node.role.startsWith("panel_")).length;
	const outcome = result.ok ? "completed" : result.degraded ? "degraded" : "failed";
	await options.audit?.append(auditRecord(options, outcome, panelSize, result.durationMs));
	return result;
}
